...
(() => {
  ...
  const ui=read(UI_KEY,{tab:'today',openExercise:null,openProgramDay:null,openProgramExercise:null,trainingMode:1,progressExercise:null,exerciseSearch:'',exercisePart:'all',todayMenuKey:null});
  ui.trainingMode=Math.max(1,Math.min(4,Number(ui.trainingMode||1)));
  ui.openProgramDay=null;
  ...
  function buildTrainingShareData(){
    ...
    return {
      schema:'training-chatgpt-share.v1',
  ...
  async function shareTrainingData(){
    const status=document.querySelector('#shareTrainingDataStatus');
    const setStatus=msg=>{if(status)status.textContent=msg;};
    const payload=buildTrainingShareData();
    const text=JSON.stringify(payload,null,2);
    const date=new Date().toISOString().slice(0,10);
    const file=new File([text],'training-chatgpt-share-'+date+'.json',{type:'application/json'});
    try{
      if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){
        await navigator.share({
          title:'Training app data for ChatGPT',
          text:'Trainingアプリの現在データです。',
          files:[file]
        });
        setStatus('共有シートを開きました。ChatGPTを選択してください。');
        return;
      }
    }catch(error){
      if(error?.name==='AbortError'){setStatus('共有をキャンセルしました。');return;}
    }
    try{
      await navigator.clipboard.writeText(text);
      setStatus('共有APIが使えないため、JSONをクリップボードにコピーしました。ChatGPTに貼り付けてください。');
    }catch{
      const blob=new Blob([text],{type:'application/json'});
      const url=URL.createObjectURL(blob),a=document.createElement('a');
      a.href=url;a.download=file.name;a.click();
      setTimeout(()=>URL.revokeObjectURL(url),1000);
      setStatus('共有APIを使えなかったため、共有用JSONを保存しました。ChatGPTに添付してください。');
    }
  }
  ...
  function renderTodayWorkoutInline(){
    const root=document.querySelector('#todayInlineExerciseList');
    ...
    const prescribed=(typeof adjustedItems==='function')?adjustedItems(key):[];
    const byName=new Map(prescribed.map(x=>[x.name,x]));
    root.innerHTML=day.items.map(([id,role])=>{
      const def=exerciseById(id);if(!def)return '';
      const target=targets[id]||{};
      const rx=byName.get(def.name)||{};
      const sets=rx.sets??target.sets??def.defaultSets??3;
      const reps=rx.reps??target.reps??def.reps??'';
      const rest=rx.rest||'—';
      const weight=def.weight?(target.weight===''?'重量未設定':esc(target.weight)+' kg'):(def.loadLabel||'自重');
      return '<article class="today-inline-exercise" data-exercise-id="'+id+'">'+
        '<div class="today-inline-main">'+
          '<div class="exercise-title-line">'+badge(def)+'<div><strong>'+googleImageName(def)+'</strong><small>'+esc(def.desc)+'</small></div></div>'+
          '<div class="today-inline-prescription"><strong>'+esc(String(sets))+' × '+esc(String(reps))+'</strong><span>休憩 '+esc(rest)+'</span></div>'+
        '</div>'+
        '<div class="today-inline-meta">'+
          (def.weight&&def.goalDirection!=='down'&&Number.isFinite(Number(def.goalKg))?weightProgressHtml(def,'today-weight-progress'):'<span class="exercise-weight-level muted-level">重量Lv —</span>')+
          '<span class="today-target-weight">'+weight+'</span>'+
        '</div>'+
      '</article>';
    }).join('');
    bindExerciseCardInteractions(root,()=>{renderTodayWorkoutInline();});
  }
  ...
  function wrapWorkout(){
    ...
    const wrapped=function(key){
      ...
      document.querySelectorAll('#exerciseList .exercise').forEach(el=>{
        ...
        if(title&&!el.querySelector('.muscle-badge'))title.insertAdjacentHTML('beforebegin',badge(def));
