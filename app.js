...
function renderHistory(){
  ...
  root.innerHTML=[...state.history].reverse().slice(0,30).map(h=>{
    ...
    return `<div class="history-item"><div>${new Date(h.iso).toLocaleDateString('ja-JP',{month:'short',day:'numeric',weekday:'short'})}</div><div><strong>${w?.title||h.key}${h.level?` · Lv${h.level}`:''}</strong><p>${details.join(' · ')}</p></div><span class="pill">${w?.code||''}</span></div>`;
  }).join('');
}
